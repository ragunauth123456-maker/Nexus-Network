...
function RegisterPage() {
  ...
  const [registered, setRegistered] = useState<{ nodeId: string; apiKey: string } | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setRegistered({ nodeId: result.node.id, apiKey: result.apiKey });
    } catch (err: any) {
        ...
        {registered ? (
          ...
          <div className="p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-emerald-400">Node Registered!</h2>
            </div>
            <p className="text-sm text-white/70 mb-4">
              Here is your API key. <strong>Save it now — it won't be shown again.</strong> You'll need it to manage your node and authenticate API requests.
            </p>
            <div className="p-4 rounded-xl bg-black/30 border border-white/10 mb-4">
              <code className="text-sm text-emerald-300 break-all font-mono select-all">
                {registered.apiKey}
              </code>
            </div>
            <button
              onClick={() => navigate({ to: `/nodes/${registered.nodeId}` })}
              className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-white font-semibold transition-all duration-200 cursor-pointer"
            >
              I've saved my key — Go to Node Profile
            </button>
          </div>
        ) : (
